...
class UserService {
  ...
  async addUser(user) {
    try {
      const nextTaskId = await db.connection.query(`SELECT nextval('user_details_seq')`, { type: db.connection.QueryTypes.SELECT })
      ...
      const created = await UserModel
        .findOrCreate({ where: { userName: user.userName }, defaults: userPayload });
